...
# @see #serviceInstance
  class VSLM < Connection
    ...
    def self.connect vim, opts = {}
      ...
      new(opts).tap do |vslm|
        vslm.vcSessionCookie = vim.cookie.split('"')[1]
      end
    end
    ...
    def serviceInstance
      @serviceInstance ||= VIM::VslmServiceInstance self, 'ServiceInstance'
    end

    # @private
    def pretty_print pp
      pp.text "VSLM(#{@opts[:host]})"
    end

    add_extension_dir File.join(File.dirname(__FILE__), 'vslm')
